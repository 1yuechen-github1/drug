;(() => {
  const render = ({
    ...
    layui.use(['laydate', 'layer', 'jquery', 'artTemplate'], function() {
      ...
      //弹出层
      const moudle = (chooseData, data, directConfirm = false) => {
        if (directConfirm) {
          // 直接执行确认逻辑
          let selectedDate = chooseData.time; // 获取选择的日期
          saveData(data, chooseData, selectedDate); // 传递选择的日期给saveData函数
        } else {
